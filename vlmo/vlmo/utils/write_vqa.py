...
def make_arrow(root, dataset_root):
    if root != "/tmp/datasets/gqa":
        ...
        make_arrow_gqa(root, dataset_root)


def make_arrow_gqa(root, dataset_root):
    with open(f"{root}/gqasubset1000.json", "r") as fp:
        questions_test_gqa_val1000 = json.load(fp)

    annotations = dict()

    for split, questions in zip(
        ["val1000"], [questions_test_gqa_val1000],
    ):
        _annot = defaultdict(dict)
        for q in tqdm(questions):
            image_id = q["image"][:-4]
            _annot[image_id][q["question_id"]] = [q["question"]]
    ...
    for split, annots in zip(
        ...
        for q in tqdm(annots):
            ...
            image_id = q["image"][:-4]
            _annot[image_id][q["question_id"]].append(
                {"labels": labels, "scores": scores,}
            )
    ...
    for split in [
        ...
        annot_paths = [
            path
            for path in paths
            if path.split("/")[-1].split("_")[-1][:-4] in annot
        ]
